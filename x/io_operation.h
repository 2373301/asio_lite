...
protected:
	typedef void (*func_type)(io_service*, io_operation*, int error, unsigned int bytes_transferred);
...
{
public:
	io_operation0(Handler h)
		: h_(h)
        , io_operation(&io_operation0::do_complete)
    {}

	static void do_complete(io_service* owner, io_operation* base,
        int error, unsigned int bytes_transferred)
	{
		io_operation0* op = static_cast<io_operation0*>(base);
...
{
public:
	io_operation1(Handler h) 
        : h_(h)
        , io_operation(&io_operation1::do_complete)
    {}
        ...
        int error, unsigned int bytes_transferred)
	{
        ...
        int err = op->error_ != 0 ? op->error_ : error;
		binder1_t<Handler, int> handler(op->h_, err);
        delete op;
		if (owner)
		{
			handler();
...
// 类似io_operation1, 执行GetAcceptExSockaddrs, 解析remote addr
template <typename Handler>
