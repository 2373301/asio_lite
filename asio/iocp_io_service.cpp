...
accept_ex_fn iocp_io_service::accept_ex_;
accept_ex_socketaddrs_fn iocp_io_service::accept_ex_socketaddrs_;
connect_ex_fn iocp_io_service::connect_ex_;
...
struct work_finished_on_block_exit
{
	...
	iocp_io_service* io_service_;
};

iocp_io_service::iocp_io_service()
	: timer_event_(0)
	, shutdown_(0)
	, stopped_(0)
...
void iocp_io_service::work_started()
{
	::InterlockedIncrement(&outstanding_work_);
}

// Notify that some work has finished.
void iocp_io_service::work_finished()
{
...
int iocp_io_service::register_handle(asio::socket& s)
{
...
void iocp_io_service::reset()
{
	::InterlockedExchange(&stopped_, 0);
}


void iocp_io_service::update_timeout()
{
...
unsigned int iocp_io_service::cancel_timer(deadline_timer& timer)
{
...
void iocp_io_service::on_pending(io_operation* op)
{
...
void iocp_io_service::on_completion(io_operation* op, DWORD last_error, DWORD bytes_transferred)
{
	op->ready_ = 1;
...
void iocp_io_service::post_immediate_completion(io_operation* op)
{
	work_started();
	post_deferred_completion(op);
}

void iocp_io_service::post_deferred_completion(io_operation* op)
{
	op->ready_ = 1;
...
void iocp_io_service::post_deferred_completions(op_queue<io_operation>& ops)
{
...
long long iocp_io_service::get_time()
{
	_timeb timebuffer;
	_ftime(&timebuffer);
...
void iocp_io_service::timer_thread(void)
{
...
void iocp_io_service::shutdown_service()
{
...
accept_ex_fn iocp_io_service::get_accept_ex(socket& s)
{
...
accept_ex_socketaddrs_fn iocp_io_service::get_accept_ex_socketaddrs(socket& s)
{
...
connect_ex_fn iocp_io_service::get_connect_ex(socket& s)
{
